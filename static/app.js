...
class DNSManager {
    ...
    async openMonitorModal() {
        this.editingMonitorId = null;
        await this.showMonitorModal({
            name: '',
            account_name: '',
            zone_id: '',
            cf_domain: '',
    ...
    // ... (keep openRestoreModal, hideRestoreModal, confirmRestore, openScheduleSwitchModal, hideScheduleSwitchModal, saveScheduleSwitch as is) ...

    async showMonitorModal(monitor) {
        ...
        if (accountSelect) {
            ...
            try {
                ...
                this.showNotification('加载账号列表失败', 'error');
            }
        }
        ...
        document.getElementById('monitor-check-type').value = monitor.check_type || monitor.type || 'ping';
        document.getElementById('monitor-check-target').value = monitor.check_target || monitor.target || '';
        ...
        document.getElementById('monitor-failure-threshold').value = monitor.failure_threshold ?? monitor.retries ?? 3;
        ...
        document.getElementById('monitor-timeout-seconds').value = monitor.timeout_seconds ?? monitor.timeout ?? 2;
    ...
    async submitMonitorForm() {
        ...
        const payload = {
            ...
            retries: Number(document.getElementById('monitor-failure-threshold').value) || 3,
